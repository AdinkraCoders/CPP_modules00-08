...
/*   Updated: 2022/11/04 16:11:00 by euyi             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if ( _grade > 0 && _grade <= form.getMinExec() && form.getSign() )
	{
		form.subExecute();
		std::cout << _name << " executed " << form.getName() << std::endl;
	}
	else if ( _grade > 0 && _grade <= form.getMinExec() && !form.getSign() )
		std::cout << _name << " failed to execute " << form.getName()
				<< " because the form is not yet signed!" << std::endl;
